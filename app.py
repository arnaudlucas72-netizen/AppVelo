...
if f_gpx is not None:
    try:
        ...
        if pts and pts != st.session_state.pts_gpx:
            ...
            # Détection et mise à jour immédiate
...
# --- 5. AFFICHAGE (Utilise la session_state pour le titre) ---
...
if weather and 'hourly' in weather:
    ...
    for i, h in enumerate(heures):
        ...
        with cols[i]:
            st.markdown(f'<div style="text-align:center; border:1px solid #ddd; padding:20px; border-radius:15px; background:#fcfcfc;"><h3 style="margin:0;">{h}h00</h3><h1 style="color:{couleur};">{score}/100</h1><p>🌡️ <b>{temp}°C</b><br>💨 {vent} km/h</p></div>', unsafe_allow_html=True)
...
if st.sidebar.checkbox("🔓 Accès Membre"):
    ...
    if u and p:
        try:
            ...
            # BOUTON TOUJOURS LÀ
            if st.sidebar.button("➕ Créer/Mettre à jour compte"):
                ...
                st.sidebar.success("Compte enregistré !")
                st.rerun()
